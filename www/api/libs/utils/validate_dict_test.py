...
@pytest.mark.parametrize('data, vk_options, expected', [
    ...
    ({'name': 123}, [VKOptions('name', str, True)], [
     'name must be of type <class \'str\'>']),  # Type error
    ({'name': 'John Doe'}, [VKOptions('name', str,
     True, options=['John Doe'])], []),  # No errors
    ({'name': 'Jane Doe'}, [VKOptions('name', str, True, options=['John Doe'])], [
     'name must be in [\'John Doe\']'])  # Options error
])
...
@pytest.mark.parametrize('data, vk_options, expected', [
    ({}, [VKOptions('name', str, default='John Doe')],
     {'name': 'John Doe'}),  # Default value
    ({'name': 'John Doe'}, [VKOptions('name', str, default='Jane Doe')], {
     'name': 'John Doe'}),  # No errors
])
